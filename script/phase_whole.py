...
parser = ArgumentParser(description="HLAPro.",prog='python3 phase_whole.py',usage=Usage)
    ...
    if len(sys.argv)==1:
        ...
        if len(snp_list)==0:
            ...
            if args.phase_flag == True:
                ...
                order = '%s/../bin/extractHAIRS_spec --indels 1 --ref %s --bam %s --VCF %s --out %s/fragment.file'%(sys.path[0], hla_ref, bamfile, my_new_vcf, outdir)
                os.system(order)
                ...
                order = '%s/../bin/SpecHap -w 15000 --vcf %s --frag %s/fragment.sorted.file --out %s/specHap.phased.vcf'%(sys.path[0],my_new_vcf, outdir, outdir)
                os.system(order)
                convert(outdir, gene, '%s/specHap.phased.vcf'%(outdir), seq_list, snp_list)
                os.system('%s/../bin/tabix -f %s'%(sys.path[0], my_new_vcf))
            ...
            if args.phase_flag == True:
                seqlist = read_spechap_seq('%s/%s.vcf.gz'%(outdir, gene))
