...
sub exon_blast{
    foreach my $class(@hlas){
        ...
        for(my $i=1;$i<=$k;$i++){
                ...
                `echo ">$class.$i" >$workdir/$class.temp.fasta`;
                `less $fa |grep -v ">" >>$workdir/$class.temp.fasta`;
                my $seq = `less $fa|grep -v ">"`; chomp $seq; $seq=~s/\s+//g;
                `$bin/blastn -query $workdir/$class.temp.fasta -out $workdir/$class.blast.out1 -db $ref -outfmt 6 -num_threads 4 -max_target_seqs 1000 `;
                open BIN, "$workdir/$class.blast.out1" or die "$!\n";
