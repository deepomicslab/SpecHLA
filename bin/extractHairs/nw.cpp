...
double nw(char* str1, char* str2,int VERBOSE)
{
	...
	int i,j;
	char* h = (char *) calloc(strlen(str1)+2,sizeof(char)); /* prepend gap chars*/
	sprintf(h, "%c%s", '_', str1);
	int len_h = strlen(h); //define to save on fn calls
	if (VERBOSE) printf("h: %s\n", h);
	char* v = (char *) calloc(strlen(str2)+2,sizeof(char));
	...
	xy** m = (xy**) calloc(len_v, sizeof(xy*)); //backtracking
	xy** ix = (xy **) calloc(len_v, sizeof(xy*));
	xy** iy = (xy **) calloc(len_v, sizeof(xy*));

	for(i = 0; i < len_v; i++) { //calloc
		m[i] = (xy*) calloc(len_h, sizeof(xy));
		ix[i] = (xy*) calloc(len_h, sizeof(xy));
		iy[i] = (xy*) calloc(len_h, sizeof(xy));
	}
